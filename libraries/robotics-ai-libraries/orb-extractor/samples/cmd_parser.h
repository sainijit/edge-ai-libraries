...
static const char num_of_images_message[] =
  "Number of images or number of cameras. Default value: 1";
...
bool ParseCommandLine(int argc, char * argv[])
{
  ...
  if ((FLAGS_images <= 0) || (FLAGS_images > 16)) {
    std::cerr << "Error:Number of images must be greater than 0 and not "
                 "greater than 16"
              << std::endl;
    return false;
  }
  ...
  if ((FLAGS_threads <= 0) || (FLAGS_threads > 16)) {
    std::cerr << "Error:Number of threads must be greater than 0 and not "
                 "greater than 16"
              << std::endl;
    return false;
  }
