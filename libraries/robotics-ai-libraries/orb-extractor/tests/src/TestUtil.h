...
inline float sin(float v) { return util::cos(_PI_2 - v); }
};  // namespace util
...
void readTestInput(cv::Mat & src, string filename, const int * size, const int dim, int type)
{
  ...
  // Create Opencv mat structure for image dimension. For 8 bit bayer, type
  // should be CV_8UC1.
...
void orbDescCPU(
  cv::Mat & image, vector<cv::KeyPoint> & keypoints, cv::Mat & descriptors,
  const vector<cv::Point> & pattern, int descriImgsize)
{
...
void computeOrientation(
  const cv::Mat & image, vector<cv::KeyPoint> & keypoints, const vector<int> & umax)
{
  for (vector<cv::KeyPoint>::iterator keypoint = keypoints.begin(), keypointEnd = keypoints.end();
