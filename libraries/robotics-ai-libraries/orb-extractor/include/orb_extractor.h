...
public:
  ...
  //! Wrapper function for extract keypoints and each descriptor of them for
  //! mono input or stereo
#ifdef OPENCV_FREE
  void extract(
    const std::vector<Mat2d> & in_image, const std::vector<Mat2d> & in_image_mask,
    std::vector<std::vector<KeyPoint>> & keypts, std::vector<Mat2d> & out_descriptors);
#else   // #ifdef OPENCV_FREE
  void extract(
    const cv::_InputArray & in_image, const cv::_InputArray & in_image_mask,
    std::vector<std::vector<cv::KeyPoint>> & keypts, const cv::_OutputArray & out_descriptors);
  ...
  void set_max_keypoints_gpu_buffer_size(
