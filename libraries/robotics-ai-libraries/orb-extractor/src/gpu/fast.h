...
public:
  ...
  static Ptr New(
    CommandListIndex index, const size_t max_nms_keypts, const size_t max_fast_keypts,
    const uint32_t num_image, CmBase::Ptr gpuObj)
  {
  ...
  Fast(
    const size_t max_nms_keypts, const size_t max_fast_keypts, const uint32_t num_image,
    CmBase::Ptr gpuObj)
  : KernelBase(kFast, gpuObj),
    nms_wait_event_(nullptr),
    fast_post_event_(nullptr),
    nms_post_event_(nullptr),
    nms_on_(true)
  {
    ...
    // Workaround kernel. Remove once zeCommandListAppendMemoryFill memory leak
    // resolve
  ...
  void CreateKernel(
    Image8u & src_image, Vec8u & src_buffer, Vec8u & mask_image, const double scale_factor,
    const bool mask_check, const uint32_t mask_step, const uint32_t ini_threshold,
    const uint32_t min_threshold, const uint32_t edge_clip, const uint32_t overlap,
    const uint32_t cell_size, const uint32_t num_image, bool nmsOn = true);
