...
public:
  orb_extractor_impl()
  {
    ...
    // gpu::Image8u img(100, 100);
    // DevImage<float> img2(200, 200);
  }
  ...
  template <typename InT, typename OutT>
  void extract(
    const InT & in_image, const InT & in_image_mask, std::vector<std::vector<KeyType>> & keypts,
    OutT & out_descriptors);

  //! calculate gpu buffer size
  ...
  void compute_keypoints_oct_trees(
    std::vector<std::vector<std::vector<gpu::PartKey>>> & raw_keypts_to_distribute,
  ...
  std::vector<gpu::PartKey> distribute_keypoints_via_tree(
  ...
  std::list<orb_extractor_node<gpu::PartKey>> initialize_nodes(
    const std::vector<gpu::PartKey> & keypts_to_distribute, const int min_x, const int max_x,
  ...
  std::vector<gpu::PartKey> find_keypoints_with_max_response(
